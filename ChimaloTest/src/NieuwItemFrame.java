...
public class NieuwItemFrame extends JFrame {
	...
	public NieuwItemFrame(Model m, Erfgoed erfgoed) {
		model=m;
		parent=this;
		setTitle("Nieuw item");
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		...
		for (int i = 0;i<model.getErfgoeden().size();i++){
			cbErfgoed.addItem( model.getErfgoeden().get(i));
				if(model.getErfgoeden().get(i).getNaam().equals(erfgoed.getNaam()))
					cbErfgoed.setSelectedIndex(i+1);
		}
		cbErfgoed.setEnabled(false);
		contentPane.add(cbErfgoed);
